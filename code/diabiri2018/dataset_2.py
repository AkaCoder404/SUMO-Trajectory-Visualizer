...
for k in range(len(UserNon)):
    InputFile = "../../data/geolife/CombinedData/combined_" + str(UserNon[k]) + ".plt"
    table = []
    try:
        with open(InputFile, 'rb') as inp:
            next(inp) # skip the first line
            for row in inp:
                row = row.rstrip()
...
# 2.Modify the labels file and create array with start_time, end_time in days and labels
    InputFile = "../../data/geolife/CombinedData/labels_" + str(UserNon[k]) + ".txt"
    table = []
    with open(InputFile, 'rb') as inp:
        next(inp) # skip the first line
        for row in inp:
